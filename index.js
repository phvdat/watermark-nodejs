...
// const webpToJimp = require('./helper/webpToJimp');
...
app.post('/process', upload.single('excelFile'), async (req, res) => {
  ...
  try {
    for (const row of rows) {
      ...
      for (let i = 0; i < imageUrls.length; i++) {
        ...
        try {
          ...
          const logoBuffer = await axios.get(logoUrl, {
            responseType: 'arraybuffer'
          });
          ...
          // const logoImage = await webpToJimp(logoUrl, '/temp');
          // const originalImage = await webpToJimp(imageUrl, '/temp');
          ...
          exifData['0th'][piexif.ImageIFD.Rating] = Math.floor(
            Math.random() * 3 + 3
          );
          ...
          const updatedJpegData = piexif.insert(
            updatedExifData,
            jpegData.toString('binary')
          );
    ...
    const zipFileName = `media/images-${Date.now()}.zip`;
